...
En la serie deestacionalizada se seguía viendo un cambio brusco, además en la tendencia la modifica un poco antes y después del outlier haciendo sea distinta, también el cambio más grande fue en la estacionalidad ya que el cambio brusco lo repetía las 5 veces que hay un ciclo cuando solo ocurrió una vez, cambiando así bastante los componentes de la serie.
...
Si hay una diferencia entre un outlier en medio de la serie o al final al momento de desestacionalizarla. Si comparamos la serie desestacionalizada sin outlier (original) contra la del outlier en medio vemos que además de existir una gran diferencia justo en un punto, varias partes de la gráfica cambian, como algunas variaciones que no existían además del cambio en toda la estacionalidad. 

En cambio la serie con outlier al final, la serie ya desestacionalizada se parecen bastante y hacia el final de esta cambia, la tendenccia mantiene una forma muy similar pero se ve ligeramente más inclinada hacia arriba que sin el outlier cuando se acerca al final, pero la más distinto es que a diferencia de un outlier en medio, en este caso no cambia la estacionalidad y se mantiene igual, por lo que afecta más a la serie que exista un outlier en medio de los datos.

```{r}
# Otra manera para cambiar observación
#tabla2 <- tabla
#tabla2$value[15] <- tabla$value[15] + 500
#tabla2
```
