...
datos |> 
  ...
  forecast(h = "14 days") |> 
  autoplot(datos, level = NULL) + xlab("Fecha") + ggtitle("Mean Box-Cox")
...
datos_fc2 |> 
  filter(.model == "snaive") |> 
  autoplot() + xlab("Fecha") + ggtitle("Seasonal Naive")
...
sna_p <- sna |> 
  select(c(Date, .mean)) |> 
  autoplot(.mean) + ggtitle("Predicción Seasonla Naive") + xlab("Fecha")
